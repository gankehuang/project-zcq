<template>
  ...
  <Card class="PolicyInfoBox4" v-bind:class="{'ImgNews-l':imgNews}" :style="style">
    ...
    <ul class="infolist" style="margin-top: -15px;">
      <li class="item" v-for="item in infolist">
        <!-- <router-link class="link-text" :key="item.Id" :to="{name:'literaturedetailthree', params:{ articleid:item.Title, unique_code: item.OnlyIndex}}"> -->
          ...
          <!-- <b class="title text-nowrap">{{ item.Title }}<span class="time">{{ item.Author }}</span></b>  -->
...
function getInfoListByClassId(_this,first){
    ...
    const prommise_getInfoList = _this.global.$post({
      '_this': _this,
      'url': _this.global.API.Policy.GetAllPolicyLaws,
      'data': {
        draw: 0,
        start: 0,
        length: _this.length,
