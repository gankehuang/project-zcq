<template>
  ...
  <div class="PolicyInfoBox" v-bind:class="{'ImgNews-l':imgNews}" :style="style" >
    <p slot="title" v-if="infoName">
      <Icon :type="iconType || 'pricetags'"></Icon>
      {{infoName}}
    </p>
    <router-link tag="a" :to="{path:moreLink, params:{name:infoName}} || '/'" slot="extra" v-if="infoName" class="link-more">更多<Icon class="icon" type="arrow-right-b"></Icon></router-link>
    <!-- <div v-if="infoName">
    ...
    <ul class="infolist" style="padding-left:25px;">
      <li class="item" v-for="item in infolist">
        <router-link target="_blank" class="link-text" :key="item.Id" :to="{name:'policydetailwithtype', params:{typeid:type, articleid:item.Id, name:item.Name}}">
          <!-- <Icon class="icon" type="arrow-right-a" /> -->
          <h3 class="title text-nowrap"><span v-html="item.Name"></span><span class="time" style="color: lightgray">{{ item.CreateDate }}</span></h3>       
...
function getInfoListByClassId(_this, first){
    ...
    const prommise_getInfoList = _this.global.$post({
      ...
      'data': {
        ...
        length: _this.length,
        //lid: _this.classId,
        retrievalInfo: JSON.stringify({'type': _this.classId=='' ? '': _this.type ,'labelid': _this.classId, 'literatureName': _this.searchkey ? _this.searchkey : ''}),
