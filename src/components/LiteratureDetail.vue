      ...
      <Card class="block-l content" id="block-l">
          ...
          <div class="layout-content">
            ...
            <div class="content clearfix">
              <ul class="nav"> 
                <li><span id="property">{{detail.Year}}</span></li> 
              ...
              <Collapse active-key="1" accordion>
                <Panel key="1" style="text-align: left;">
                  ...
                  <p slot="content" v-if="detail.tCountry" >
                    <span class="itemlist">目标国家与区域：<router-link v-for="item in detail.tCountry" :key="item.countryID" :to="{name:'countrydetail', params:{countryid:item.countryId}}" class="value">{{item.countryName}} ({{item.countryArea}})</router-link></span>
                  </p>
                  <p slot="content" v-if="detail.pCountry">
                    <span class="itemlist">出版地：<router-link v-for="item in detail.pCountry" :key="item.countryID" :to="{name:'countrydetail', params:{countryid:item.countryId}}" class="value">{{item.countryName}} ({{item.countryArea}})</router-link></span>
                  </p>
                  <p slot="content" v-if="detail.wCountry">
                    <span class="itemlist">撰写国家与区域：<a v-for="item in detail.wCountry" :key="item.countryID" :to="{name:'countrydetail', params:{countryid:item.countryId}}" class="value">{{item.countryName}} ({{item.countryArea}})</a></span>
                  </p>
                  <p slot="content" v-if="detail.Label">
                    <span class="itemlist">标签：<a v-for="item in detail.Label" :key="item.labelid" class="value">{{item.labelname}}</a></span>
                  </p>
                  ...
                  <p slot="content" v-if="detail.PublishDate"><span class="itemlist">出版时间：{{detail.PublishDate}}</span></p>       
                </Panel>
              </Collapse>
            </div> -->
  ...
  methods: {
    'getPolicyDetail':function() {
      ...
      /*const prommise_getPolicyDetail = _this.global.$post({
        '_this': _this,
        'url': 'http://10.200.32.96/document/html?file_name=' + _this.$route.params.title,
        'data': { literatureID: _this.$route.params.title },
        'f_resolve': function(data){
            console.log('%%%%%%%%%%%%%%%%',data);
            var newData = data;
            newData.Label = JSON.parse(data.Label);
            newData.pCountry = JSON.parse(data.pCountry);
            newData.ImagePath = _this.global.normalResourceUrl(data.ImagePath);
        ...
        //_this.getApiData( 'http://10.200.32.96:8888/document/html?file_name=' + _this.$route.params.title+'&table_flag='+ _this.$route.params.table_flag );
        _this.getApiData( _this.global.Domain.api1 + '/document/html?file_name=' + _this.$route.params.title+'&table_flag='+ _this.$route.params.table_flag );
