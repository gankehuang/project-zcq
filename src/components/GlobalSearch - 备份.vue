  ...
  <div class="GlobalSearch">
    <div class="container-main clearfix">
        <div class="info-box top" style="">
            <Card :bordered="true" dis-hover style="height: 390px;">
              ...
              <Tabs type="card">
                  ...
                  <Tab-pane label="高级检索">
                      <div style="width: 100%; height: 320px; display: inline-block; float:left; padding: 0px 10px;">
                        ...
                        <Select v-model="type" filterable placeholder="请选择" style="width:10%; margin-top: 5px;">
                            <Option value="and">并且</Option> 
                            <Option value="or">或者</Option>
                        </Select>
                        <Select v-model="headline2" filterable placeholder="请选择标题" style="width:15%; margin-top: 5px;">
                            <Option value="标题">标题</Option> 
                            <Option value="关键词">关键词</Option>
                            <Option value="责任者">作者</Option>
                            <Option value="责任者机构">作者机构</Option>
                        ...
                        <Input v-model="headlineInt2" placeholder="请输入..." style="width:30%; margin-top: 5px;" />
                        
                        <Tabs size="small">
                            <TabPane label="信息来源">
                                <CheckboxGroup v-model="informationSource">
                                    <Checkbox :label="key" v-for="value,key in field.信息来源全称[0]" style="width: 200px; height: 20px; float:left;white-space:nowrap;text-overflow:ellipsis;-o-text-overflow:ellipsis;overflow: hidden;" :title="item"><span v-if="value">[{{value}}]</span><span>{{key}}</span></Checkbox>
                                </CheckboxGroup>
                            </TabPane>
                            <TabPane label="作品语种">
                ...
                <!-- <i-input style="width: 80%; margin-left: 10%" v-model="value">
                    <span slot="prepend">一级检索</span>
                    <i-button type="success" slot="append" icon="ios-search" @click="fullTextSearch()"></i-button>
                </i-input><br/> -->
                <!-- <i-input style="width: 80%; margin-left: 10%" v-model="catalogueValue">
                    <span slot="prepend">目录检索</span>
                    <i-button type="success" slot="append" icon="ios-search" @click="catalogueSearch()"></i-button>
                </i-input><br/> -->
        ...
        <!-- <div class="info-box second">
          <i-form :model="formItem" :label-width="80">
            ...
            <p style="text-align: left; background: #026EB8; font-size: 14px; padding: 5px; color: white">&nbsp;&nbsp;作品语种</p>
            <div style="padding: 10px;">
              <Checkbox-group :model.sync="formItem.language" v-for="(Lanitem, index) in field.作品语种">
                <Checkbox :value="Lanitem" :key="index"><span>{{Lanitem}}</span></Checkbox>
              </Checkbox-group>
            </div>
        
            <p style="text-align: left; background: #026EB8; font-size: 14px; padding: 5px; color: white">&nbsp;&nbsp;主要国家</p>
            <div style="padding: 10px;">
              <Checkbox-group :model.sync="formItem.country" v-for="(Englanditem, index) in field.出版国">
                <Checkbox :value="Englanditem"><span>{{Englanditem}}</span></Checkbox>
              </Checkbox-group>
            </div>
            
            <p style="text-align: left; background: #026EB8; font-size: 14px; padding: 5px; color: white">&nbsp;&nbsp;信息来源</p>
            <div style="padding: 10px;">
              <Checkbox-group :model.sync="formItem.information" v-for="(Sourceitem, index) in field.信息来源全称">
                <Checkbox :title="Sourceitem" :value="Sourceitem" style="display: block; height: 20px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><span>{{Sourceitem}}</span></Checkbox>
              </Checkbox-group>
            </div>
        
            <p style="text-align: left; background: #026EB8; font-size: 14px; padding: 5px; color: white">&nbsp;&nbsp;字段检索</p>
            <div style="padding: 10px;">
                <Row prop="select">
                  <i-select v-model="formItem.select" placeholder="请选择字段">
                      <i-option value="标题">标题</i-option>
                      <i-option value="关键词">关键词</i-option>
                      <i-option value="作者">作者</i-option>
                      <i-option value="作者机构">作者机构</i-option>
                      <i-option value="摘要">摘要</i-option>
                      <i-option value="主题国">主题国</i-option>
              ...
              <i-form v-ref:form-dynamic :model="formDynamic" :label-width="80">
                ...
                <Row v-for="(item, index) in formDynamic.items" :prop="'items.' + index + '.value'" :rules="{required: true, message: '项目' + (index + 1) +'不能为空', trigger: 'blur'}">
                    <i-col span="18">
                        <i-input type="text" :value.sync="formDynamic.input" placeholder="请输入..."></i-input>
                    </i-col>
                ...
                <i-button type="dashed" long @click="handleAdd" >新增</i-button>
                <i-button type="primary" long @click="handleSubmit()">检索</i-button>
              </i-form>
        ...
        <div class="info-box1 second searchresult">
            <ul class="countryinfolist" v-if="countryLiterature">
              ...
              <li class="item" v-for="(item, index) in countryLiterature" style=" height: 40px;">
                <router-link class="link-text" :key="item.id" :to="{name:'literaturedetailtwo', params:{title: item.file_name, name: item.标题, table_flag: item.table_flag, id: item.id,zrz:item.责任者,xxlyqc:item.信息来源全称, xxlymc:item.信息来源名称,wxjh:item.文献集合, ybkh: item.印本刊号,dzkh:item.电子刊号,dzcbwsh:item.电子出版物书号, year: item.发布年,jq:item.期,ys:item.页数, ztg:item.主题国,gjc:item.关键词,zy: item.摘要,zpyz:item.作品语种,zrzjg:item.责任者机构 } }" style="height:40px;">
            ...
            <!-- 全文检索 -->
              <div class= "list" v-if="searchtext&&!single">
                <p style="height: 30px; line-height:30px; background:#026EB8">
                  <span style="width: 3%; text-align:center;display:inline-block;color:white;font-size:14px;">序号</span>
                  <span style="font-size:14px;color:#8EC31F; width:60%; display:inline-block;color:white">标题</span>
                  <span style="display: inline-block; float:right;margin-left: 10px; width: 20%;height:20px;color:white;font-size:14px;">来源平台</span>
                ...
                <router-link v-for="(item, index) in searchtext" class="link-text" :key="item.id" :to="{name:'literaturedetailtwo', params:{title: item.file_name, name: item.标题, table_flag: item.table_flag, id: item.id,zrz:item.责任者,xxlyqc:item.信息来源全称, xxlymc:item.信息来源名称,wxjh:item.文献集合, ybkh: item.印本刊号,dzkh:item.电子刊号,dzcbwsh:item.电子出版物书号, year: item.发布年,jq:item.期,ys:item.页数, ztg:item.主题国,gjc:item.关键词,zy: item.摘要,zpyz:item.作品语种,zrzjg:item.责任者机构 } }" style="padding: 3px 10px;">
                    <p style="">
                      <span style="width: 3%; text-align:center;display:inline-block">{{index+1}}</span>
                      <span v-html="item.标题" style="font-size:16px;color:#8EC31F; width:60%; display:inline-block"></span>
                      <span style="display: inline-block; float:right;margin-left: 10px; width: 20%;height:20px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;-o-text-overflow:ellipsis;"><span v-if="item.信息来源名称">[{{item.信息来源名称}}]</span>{{item.信息来源全称}}</span>
                    ...
                    <span v-html="item.content" style="display: inline-block;width: 60%;margin-left:3%;white-space:nowrap;text-overflow:ellipsis;-o-text-overflow:ellipsis;overflow: hidden;"></span>
              ...
              <!-- <Card class = "searchresult" style=""> -->
                <div class= "list" v-if="erJirestData">
                  <p style="height: 30px; line-height:30px; background:#026EB8">
                    <span style="width: 3%; text-align:center;display:inline-block;color:white;font-size:14px;">序号</span>
                    <span style="font-size:14px;color:#8EC31F; width:60%; display:inline-block;color:white">标题</span>
                    <span style="display: inline-block; float:right;margin-left: 10px; width: 20%;height:20px;color:white;font-size:14px;">来源平台</span>
                  ...
                  <router-link v-for="(item, index) in erJirestData" class="link-text" :key="item.id" :to="{name:'literaturedetailtwo', params:{title: item.file_name, name: item.标题, table_flag: item.table_flag, id: item.id,zrz:item.责任者,xxlyqc:item.信息来源全称, xxlymc:item.信息来源名称,wxjh:item.文献集合, ybkh: item.印本刊号,dzkh:item.电子刊号,dzcbwsh:item.电子出版物书号, year: item.发布年,jq:item.期,ys:item.页数, ztg:item.主题国,gjc:item.关键词,zy: item.摘要,zpyz:item.作品语种,zrzjg:item.责任者机构 } }" style="padding: 3px 10px;">
                      <p style="">
...
export default {
  ...
  methods: { 
              ...
              success: function(data){
                 //console.log(data);
                  ...
                  data.resultData.forEach(function(val, index) {
                    let str = val.标题;
                    if(str){
                      let newstr = str.replace(reg, "<font color=red>"+ _this.value +"</font>");
                      val.标题 = newstr;
                    }
                    let str2 = val.title;
                    if(str2){
                      let newstr2 = str2.replace(reg, "<font color=red>"+ _this.value +"</font>");
                      val.title = newstr2;
                    }
                    let str1 = val.content;
                    if(str1){
                      let newstr1 = str1.replace(reg, "<font color=red>"+ _this.value +"</font>");
                      val.content = newstr1;
                    }
                 });
      ...
      handleRemove (item) {
        const _this = this;
        console.log(item);
          _this.formItem.items.splice(item);
      },
      setLocalStorage() {
        const _this = this
        let keyWordList = [
